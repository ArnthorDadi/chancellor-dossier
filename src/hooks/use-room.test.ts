...
describe('useRoom Hook', () => {
  ...
  beforeEach(() => {
    ...
    mockGameLogic.assignRoles.mockReturnValue({ user123: { role: 'LIBERAL', party: 'LIBERAL' } } as any)
  })
  ...
  describe('createNewRoom', () => {
    it('should create a new room successfully', async () => {
      ...
      await expect(result.current.createNewRoom('Test Room')).resolves.toBe('TEST1')
      
      expect(mockRealtimeDb.generateUniqueRoomCode).toHaveBeenCalled()
      expect(mockRealtimeDb.createRoom).toHaveBeenCalledWith('TEST1', expect.objectContaining({
        id: 'TEST1',
    ...
    it('should throw error if user is not authenticated', async () => {
      ...
      await expect(result.current.createNewRoom()).rejects.toThrow('Must be authenticated to create a room')
    })
    ...
    it('should handle room creation errors', async () => {
      ...
      await expect(result.current.createNewRoom()).rejects.toThrow('Database error')
    })
  })
  ...
  describe('joinRoom', () => {
    it('should join an existing room successfully', async () => {
      ...
      await expect(result.current.joinRoom('TEST1', 'New Player')).resolves.toBeUndefined()
    ...
    it('should throw error if room does not exist', async () => {
      ...
      await expect(result.current.joinRoom('INVALID', 'Test')).rejects.toThrow('Room not found')
    })
    ...
    it('should throw error if game is in progress', async () => {
      ...
      await expect(result.current.joinRoom('TEST1', 'Test')).rejects.toThrow('Cannot join a game in progress')
    })
    ...
    it('should throw error if room is full', async () => {
      ...
      await expect(result.current.joinRoom('TEST1', 'Test')).rejects.toThrow('Room is full')
    })
  })
  ...
  describe('startGame', () => {
    ...
    it('should throw error if user is not admin', async () => {
      ...
      await expect(result.current.startGame()).rejects.toThrow('Only admin can start the game')
    })
    ...
    it('should throw error if game cannot be started', async () => {
      ...
      mockGameLogic.canStartGame.mockReturnValue({ canStart: false, reason: 'Not enough players' })
      
      const { result } = renderHook(() => useRoom('TEST1'))
      
      await expect(result.current.startGame()).rejects.toThrow('Not enough players')
    })
  })
