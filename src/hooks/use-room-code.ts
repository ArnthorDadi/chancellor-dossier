...
export const useRoomCode = (): UseRoomCodeReturn => {
  ...
  const generateCode = useCallback(async (): Promise<string> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to generate room code";
  ...
  const validateCode = useCallback(async (code: string): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to validate room code";
      setError(errorMessage);
      return false;
    } finally {
      setLoading(false);
