...
describe('DigitalEnvelope', () => {
  ...
  describe('when role and party are provided', () => {
    ...
    it('uses controlled open state when provided', () => {
      ...
      render(<DigitalEnvelope {...mockProps} isOpen={true} onToggle={onToggle} />)
  ...
  describe('role card display', () => {
    it('displays LIBERAL role correctly', () => {
      ...
      expect(screen.getByText('LIBERAL')).toBeInTheDocument()
      expect(screen.getByText('Protect democracy and enact liberal policies')).toBeInTheDocument()
    })
    ...
    it('displays FASCIST role correctly', () => {
      ...
      expect(screen.getByText('Enact fascist policies and elect Hitler as Chancellor')).toBeInTheDocument()
    })
    ...
    it('displays HITLER role correctly', () => {
      ...
      expect(screen.getByText('Get elected Chancellor or enact 6 fascist policies to win')).toBeInTheDocument()
    })
  })
  ...
  describe('party card display', () => {
    it('displays LIBERAL party correctly', () => {
      ...
      expect(screen.getByText('LIBERAL PARTY')).toBeInTheDocument()
      expect(screen.getByText('Member of the Liberal Party')).toBeInTheDocument()
    })
    ...
    it('displays FASCIST party correctly', () => {
      ...
      expect(screen.getByText('Member of the Fascist Party')).toBeInTheDocument()
    })
  })
  ...
  describe('when role or party is missing', () => {
    it('shows unavailable message when role is missing', () => {
      ...
      expect(screen.getByText('ROLE INFORMATION UNAVAILABLE')).toBeInTheDocument()
  ...
  describe('instructions and styling', () => {
    it('shows instructions when envelope is closed', () => {
      ...
      expect(screen.getByText('Press OPEN ENVELOPE to reveal your secret role')).toBeInTheDocument()
      expect(screen.getByText('Keep your identity confidential from other players')).toBeInTheDocument()
    })

    it('hides instructions when envelope is open', () => {
      render(<DigitalEnvelope {...mockProps} isOpen={true} />)
      
      expect(screen.queryByText('Press OPEN ENVELOPE to reveal your secret role')).not.toBeInTheDocument()
      expect(screen.queryByText('Keep your identity confidential from other players')).not.toBeInTheDocument()
    })
  })
  ...
  describe('accessibility', () => {
    ...
    it('maintains proper semantic structure', () => {
      ...
      expect(screen.getByRole('heading', { name: 'SECRET DOSSIER' })).toBeInTheDocument()
