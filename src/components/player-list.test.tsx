...
describe('PlayerList', () => {
  ...
  it('highlights current user correctly', () => {
    ...
    const testUserElement = screen.getByText('TestUser').closest('div')?.parentElement?.parentElement
    expect(testUserElement).toHaveClass('bg-liberal-blue/10', 'border-liberal-blue')
    expect(screen.getByText('(You)')).toBeInTheDocument()
  })
  ...
  it('shows non-admin players with correct styling', () => {
    ...
    const otherUserElement = screen.getByText('OtherUser').closest('div')?.parentElement?.parentElement
    expect(otherUserElement).toHaveClass('bg-vintage-cream', 'border-noir-black')
  })
  ...
  it('shows "need more players" message when less than 5 players', () => {
    ...
    expect(screen.getByText('Need 3 more players to start')).toBeInTheDocument()
  })
  ...
  it('hides admin badge for non-admin users', () => {
    ...
    const otherUserElement = screen.getByText('OtherUser').closest('div')?.parentElement?.parentElement
    expect(otherUserElement).toHaveClass('bg-fascist-red/10', 'border-fascist-red')
  })
