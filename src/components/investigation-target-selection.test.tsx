...
describe('InvestigationTargetSelection', () => {
  ...
  it('renders investigation interface correctly', () => {
    ...
    expect(screen.getByText('As President, you may investigate one player\'s party membership')).toBeInTheDocument()
  ...
  it('shows investigate button only for eligible players', () => {
    ...
    const targetPlayerCard = screen.getByText('Target Player').closest('[data-player-id]')
  ...
  it('shows loading state during investigation', async () => {
    mockOnInvestigate.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)))
  ...
  it('applies correct styling for different player states', () => {
    ...
    const investigatedCard = screen.getByText('Already Investigated').closest('div')
  ...
  it('displays investigation protocol instructions', () => {
    ...
    expect(screen.getByText('• Select one player to investigate their party membership')).toBeInTheDocument()
    expect(screen.getByText('• Investigation results are visible only to you')).toBeInTheDocument()
    expect(screen.getByText('• Each player can only be investigated once per game')).toBeInTheDocument()
    expect(screen.getByText('• You cannot investigate yourself')).toBeInTheDocument()
  })
  ...
  it('handles investigation error gracefully', async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith('Investigation failed:', expect.any(Error))
    })
